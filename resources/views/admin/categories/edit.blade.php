...
<div class="max-w-xl">
    ...
    <form method="POST" action="{{ route('admin.categories.update', $category) }}" enctype="multipart/form-data"
        ...
        @csrf @method('PUT')
        <div class="p-6 border-b border-gray-100"><h2 class="font-semibold text-gray-800">Edit Kategori</h2></div>
        <div class="p-6 space-y-4">
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-gold-400/30 focus:border-gold-400 resize-none">{{ old('description', $category->description) }}</textarea>
            </div>
            <div>
            ...
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $category->is_active) ? 'checked' : '' }} class="w-4 h-4 accent-[#1B120D] rounded">
